...
# -*- coding: utf-8 -*-
"""
중경1공장 품질별 (YS2_STRESS - I_YS) 차이값 분포 stripplot
"""
...
def calculate_ys2_minus_iys_difference(data):
    """YS2_STRESS - I_YS 차이값 계산 및 분석"""
    
    print("\n🔢 YS2_STRESS - I_YS 차이값 계산 중...")
    ...
    # 차이값 계산
    data['ys2_minus_iys'] = data[ys2_col] - data[iys_col]
    
    print(f"📊 (YS2_STRESS - I_YS) 차이값 분석:")
    print(f"   전체 데이터: {len(data):,}개")
    print(f"   차이값 범위: {data['ys2_minus_iys'].min():.2f} ~ {data['ys2_minus_iys'].max():.2f} MPa")
    print(f"   차이값 평균: {data['ys2_minus_iys'].mean():.2f} MPa")
    print(f"   차이값 표준편차: {data['ys2_minus_iys'].std():.2f} MPa")
    
    # 양수/음수 분포
    positive_count = (data['ys2_minus_iys'] > 0).sum()
    negative_count = (data['ys2_minus_iys'] < 0).sum()
    zero_count = (data['ys2_minus_iys'] == 0).sum()
    
    print(f"\n📈 차이값 분포:")
    print(f"   양수 (YS2_STRESS > I_YS): {positive_count:,}개 ({positive_count/len(data)*100:.1f}%)")
    print(f"   음수 (YS2_STRESS < I_YS): {negative_count:,}개 ({negative_count/len(data)*100:.1f}%)")
    print(f"   0 (YS2_STRESS = I_YS): {zero_count:,}개 ({zero_count/len(data)*100:.1f}%)")
    
    return data

def create_ys2_minus_iys_stripplot(data):
    """품질별 (YS2_STRESS - I_YS) 차이값 stripplot 생성"""
    
    if data is None or len(data) == 0:
        print("❌ 데이터가 없습니다.")
        return None
    
    print("\n🎨 (YS2_STRESS - I_YS) 차이값 stripplot 생성 중...")
    
    # 컬럼 설정
    quality_col = 'p_spec'
    diff_col = 'ys2_minus_iys'
    
    # 상위 5개 품질 선정
    top_qualities = data[quality_col].value_counts().head(5)
    filtered_data = data[data[quality_col].isin(top_qualities.index)].copy()
    
    print(f"\n📋 상위 5개 품질별 (YS2_STRESS - I_YS) 차이값 분포:")
    ...
    # 제목 및 레이블 (한글 사용)
    plt.title(
        '중경1공장 필터링된 데이터: 상위 5개 품질별 (YS2_STRESS - I_YS) 차이값 분포',
        fontsize=16,
        fontweight='bold',
        pad=20
    )
    plt.xlabel('품질 (Quality)', fontsize=12, fontweight='bold')
    plt.ylabel('YS2_STRESS - I_YS (MPa)', fontsize=12, fontweight='bold')
    ...
    info_text = "품질별 차이값 (YS2_STRESS - I_YS):\n"
    ...
    # 저장
    filename = '중경1공장_상위5개품질_YS2_STRESS_minus_I_YS_차이값_stripplot.png'
...
def main():
    """메인 실행 함수"""
    print("🚀 중경1공장 품질별 (YS2_STRESS - I_YS) 차이값 분석")
    print("=" * 80)
    ...
    # 3. 차이값 계산
    if data is not None:
        data_with_diff = calculate_ys2_minus_iys_difference(data)
        
        # 4. stripplot 생성
        if data_with_diff is not None:
            filename = create_ys2_minus_iys_stripplot(data_with_diff)
            if filename:
